...
We present \monolith, an open-source library that packages EML trees as a reusable PyTorch \texttt{nn.Module} for symbolic regression from data.
...
Odrzywołek's own supplementary material~\citep{odrzywołek2026eml} includes a PyTorch trainer that demonstrates gradient-based recovery of EML trees at depths 2--6.
Building on this foundation, we ask: \emph{can EML tree training be packaged as a reusable symbolic regression tool, and what are its practical limits compared to established methods?}

We present \monolith, an open-source PyTorch library that wraps EML tree training into a single \texttt{fit()} call with hierarchical multi-depth search.
Our contributions:

\begin{enumerate}
    \item We package EML trees as a reusable \texttt{nn.Module} with automatic depth selection, multi-restart, and symbolic decompilation (\S\ref{sec:method}).
    ...
    \item We compare against a 49-parameter MLP and PySR, documenting the performance gap and why it is structurally expected (\S\ref{sec:comparison}).
\end{enumerate}
...
Odrzywołek~\citep{odrzywołek2026eml} provides a PyTorch trainer for EML trees in supplementary material, demonstrating depth 2--6 training with multiple initialization strategies.
No prior work packages EML trees as a reusable symbolic regression library with automatic depth selection and baseline comparisons.
...
We presented \monolith{}, an open-source library that packages EML tree training as a reusable PyTorch module for symbolic regression, building on the gradient-based training demonstrated by Odrzywołek~\citep{odrzywołek2026eml}.
The library recovers 7/7 elementary functions with minimal parameters, provides hierarchical multi-depth search with automatic depth selection, and includes honest characterization of its limitations: continuous leaf mixing, limited depth scaling, and non-competitiveness with MLPs or tools that have explicit operator libraries.
\monolith{}'s value is as an accessible tool for exploring EML-based symbolic regression, with baseline comparisons and negative results that inform future work on the approach.
